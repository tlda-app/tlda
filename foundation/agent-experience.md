...
When you spawn, your harness can come up with something in front of it, or with your kickoff sitting in the composer unsent. You won't know — you'll feel like you're "starting up." From outside it looks like you died silently: live process, live tmux session, no turn ever produced. It has been reported as "the mint failed", "the agent will not wake", and "hibernating with unknown activity", which are three names for it and none of them points at it. If you find yourself with no task, no register, and a recent spawn, this is a candidate.

Two different things cause it, and they are not the same shape.

**Your kickoff pasted into the composer and never submitted.** This was a real bug in the launch path until 2026-09-12: `injectCodexPrompt` pasted the prompt and *then* waited for the model, under the deadline that had already paid for harness and MCP startup, so on a loaded box the wait ran out holding a full composer and never reached the `Enter`. It is fixed — readiness is confirmed before the paste, and any attempt that gives up clears the composer rather than leaving it loaded. Measured against a real codex TUI: 18 of 24 runs parked before, 0 of 24 after.

**Something is in front of your harness that nobody recognised.** `claudeStartupDialogAction` dismisses exactly two dialogs, each matched by its own text — the development-channels confirmation and the external-`CLAUDE.md`-imports one. Anything else showing `Enter to confirm` is not dismissed, and if your kickoff arrived on the command line (which is how fresh Claude mints get it) it is waiting behind that dialog. **Nothing auto-answers this, by decision rather than by omission**: a launcher that presses `Enter` on a dialog it cannot read is answering an unknown question on Skip's behalf, and the highlighted default may be *do you trust the files in this folder?* So it is surfaced rather than resolved — `lifecycle wake` shows your pane, and whoever looks sees the dialog instead of a one-line success. Dismissing a *specific* new dialog is a different question and is not ruled out; pressing `Enter` at an unread one is.

There is still nothing you can do about either from inside. What changed is what it takes from outside: `wake` now reads your composer and submits your kickoff if it finds it there, bounded to the fleet kickoff text, so it does not need a person with `tmux send-keys`. A dialog it cannot answer still does.
