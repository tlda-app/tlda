...
test('a referenced PDF carries its tracked DVI bounding-box sidecar', t => {
  const dir = mkdtempSync(join(tmpdir(), 'tlda-tex-deps-pdf-bb-'))
  t.after(() => rmSync(dir, { recursive: true, force: true }))
  mkdirSync(join(dir, 'figures'))
  ...
  assert.deepEqual(scanTexDependencyClosure('main.tex', dir).files, [
    'figures/errors-combined-error.bb',
    'figures/errors-combined-error.pdf',
    'main.tex',
  ])
})
