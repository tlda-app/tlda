...
import { fetchDocumentManifest, loadDocumentByFormat } from '../loaders/documentFormatLoader'
...
export function useDocAutoOpen(
  ...
  const handleDocArrived = useCallback(async (event: DocArrivedEvent) => {
    ...
    let manifest
    try {
      manifest = await fetchDocumentManifest(basePath)
    } catch {
      return
    }
    const targets = projectInfo.targets?.map((target: { texBase: string; pages: number }) => ({
      name: target.texBase,
      title: target.texBase.replace(/_/g, ' '),
      pages: target.pages,
      basePath,
    }))
    const foreignDocument = await loadDocumentByFormat({ name: event.name, basePath, manifest, targets })
    ...
    for (let i = 0; i < pageCount; i++) {
      ...
      shapes.push({
        id,
        type: (manifest.view.kind === 'image-pages' ? 'zoomable-image' : htmlView ? 'html-page' : 'svg-page') as any,
        x,
        y,
        isLocked: true,
        ...
        props: {
          w: width,
          h: height,
          ...(manifest.view.kind === 'image-pages'
            ? { src: foreignPage.src, imageW: foreignPage.width, imageH: foreignPage.height }
            : htmlView
...
async function fetchForeignPage(
  ...
  try {
    const url = file ? `${basePath}${file}` : `${basePath}page-${pageIndex + 1}.svg`
