...
interface CanvasClipPanelProps {
  ...
  viewportId?: string
  /**
   * Report this panel's effective camera outward, whenever it changes.
   *
   * A layer composited over the panel has to be registered with the panel's own
   * camera, not the main editor's: the panel is a viewport with its own camera
   * over a shared store, so the two differ, and a layer using the wrong one
   * draws in the wrong place. `children` render outside the camera-transformed
   * box, so a child cannot read it from the DOM either.
   *
   * Read-only: this reports, it never sets. Nothing about the panel's behavior
   ...
   *
   * Distinct from `children`, which render as a sibling of the canvas and so do
   * not line up with it. `.clip-panel-canvas` is already `position: relative`,
   * so an `inset: 0` layer placed here covers exactly the region the panel
   * draws — which is what a layer annotating this panel's content needs, and
   * why this is not just `children`.
   */
...
export function CanvasClipPanel({
  ...
  // is recomputed each render and a new reference with the same values would
  // otherwise re-notify on every render.
  useEffect(() => {
    onCamera?.(camera)
  }, [onCamera, camera.x, camera.y, camera.z])
