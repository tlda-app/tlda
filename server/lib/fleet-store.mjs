...
export class FleetStore {
  ...
  // changes nothing anyone can see: no existing row carries it, and ~960 probe
  // rows are what makes the panel unreadable.
  //
  // Four things decide the shape, and each has a measurement behind it rather
  // than a preference:
  //
  // BATCHED. ~1,200 rows is the size this store has been burned at twice --
  // `/api/tasks/retire` caps at 100 because 500 measured ~350ms of synchronous
  // SQLite on the event loop, and the label short-circuit exists because label
  // work measured ~230ms per event over ~1,300 agents. One statement over the
  // whole set is that shape again.
  //
  // OFF THE LOOP. It yields between batches, so a slow run cannot park chat or
  // sync behind it. Same boundary `insert-does-no-maintenance.test.mjs` pins for
  // the insert path.
  //
  // RE-DERIVED, NOT LISTED. Each batch re-runs the query rather than walking a
  // snapshot, so a row that acquires the label mid-run is still caught and a
  // half-finished run is resumed by running it again. There is no cursor and no
  // "already done" flag to get wedged -- the WHERE clause is the idempotency,
  // the same way the `notify_at` rename above is idempotent.
  //
  // CONVERGENCE IS CHECKED, NOT ASSUMED. If a batch flags nothing -- a row that
  // cannot be written, say -- the same ids would come back forever. So it
  // measures progress and stops rather than spinning.
  //
  // It goes through `updateAgentMeta`, which calls `_syncAgentRegistry(id)`.
  // That is not incidental: the panel does not read this table. It reads
  // `_aliveAgentRosterView`, an in-memory registry behind a load-once latch, so
  // a bare UPDATE would report ~960 rows changed and move nothing on screen.
  //
  // Hiding is a display property. This sets one metadata field, deletes nothing,
  // marks nothing dead, and is undone by the same call with `hidden: false`.
  async backfillHiddenForLabel(label, { batchSize = 100, onBatch = null } = {}) {
    if (!label) throw new TypeError('backfillHiddenForLabel requires a label');
    const size = Math.max(1, Math.min(Number(batchSize) || 100, 100));
    // Same predicate the panel's own source uses (`_getAliveAgents`: alive, not
    // a mint shell), so "what this flagged" and "what the panel would have
    // listed" are the same set and the before/after counts are comparable.
    const pending = this.db.prepare(`
      SELECT agents.id AS id
      FROM agents
      WHERE agents.dead = 0
        AND COALESCE(json_extract(agents.metadata, '$.shell'), 0) != 1
        AND COALESCE(json_extract(agents.metadata, '$.hidden'), 0) != 1
        AND EXISTS (
          SELECT 1 FROM json_each(COALESCE(agents.labels, '[]')) WHERE value = @label
        )
      ORDER BY agents.id
      LIMIT @size
    `);
    ...
    let batches = 0;
    for (;;) {
      const ids = pending.all({ label, size }).map(row => row.id);
      if (!ids.length) break;
      for (const id of ids) this.updateAgentMeta(id, { hidden: true });
      // Confirm progress with indexed point reads on the ids just written, NOT
      // by re-running the query above. That query is a scan -- `json_each` over
      // the labels and `json_extract` over the metadata are unindexable, and it
      // measured 72ms per call over 3,000 rows. Running it twice a batch put
      // the whole scan cost on the loop twice for no extra information.
      const progressed = ids.filter(id => {
        const row = this._getAgent.get(id);
        if (!row) return false;
        try {
          return JSON.parse(row.metadata || '{}')?.hidden === true;
        } catch {
          return false;
        }
      }).length;
      flagged += progressed;
      batches += 1;
      onBatch?.({ batches, flagged, batchSize: ids.length, progressed });
      if (progressed === 0) {
        // Nothing moved, so running the same query again would return the same
        // rows forever. Report it rather than spin.
        return { flagged, batches, stalledOn: ids.slice(0, 5), converged: false };
      }
      // Give the loop back between batches.
      await new Promise(resolve => setImmediate(resolve));
    }
    return { flagged, batches, stalledOn: [], converged: true };
  }
