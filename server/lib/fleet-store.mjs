...
export class FleetStore {
  ...
  admitDaemonAgentStatusIdentities(admissions, daemonKey) {
    ...
    this.db.transaction(() => {
      ...
      const reservedNewNames = new Set();
      const planned = [];
      for (const admission of rows) {
        ...
        if (!current) {
          canonicalName = this.allocateFreshFriendlyName(name, { excludeId: id });
          if (reservedNewNames.has(canonicalName.toLowerCase())) {
            for (let suffix = 2; suffix < 10000; suffix++) {
              const candidate = this.allocateFreshFriendlyName(`${name}-${suffix}`, { excludeId: id });
              if (!reservedNewNames.has(candidate.toLowerCase())) {
                canonicalName = candidate;
                break;
              }
            }
          }
          if (!canonicalName || reservedNewNames.has(canonicalName.toLowerCase())) throw new Error(`No available friendly-name variant for "${name}"`);
          reservedNewNames.add(canonicalName.toLowerCase());
        } else if (current.dead && canonicalName) {
          const nameOwner = this._getLiveAgentRowByFriendlyName.get(canonicalName);
          if (nameOwner && nameOwner.id !== id) throw new Error(`Name "${canonicalName}" is already taken by another live agent`);
        }
      ...
      for (const { id, name, harness, current } of planned) {
        ...
        if (!current) {
          ...
          this.db.prepare(`
            UPDATE agents
            SET last_seen = ?, dead = 0,
                metadata = json_patch(COALESCE(metadata, '{}'), ?)
            WHERE id = ? AND human = 0
          `).run(now, JSON.stringify({ kind: harness }), id);
        }
