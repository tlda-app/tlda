  ...
  const tools = [
    ...
    // ---- Search & History ----
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          query: { type: 'string', description: 'Literal text terms plus event filters: from:, to:, involving:, agent:, project:, type:, role:, since:, after:, before:, id:. id: takes the number the system already gave you — inbox() prints `id:2923649` beside a message, so that string IS the query, and its canonical form id:chat#2923649 works too. A row named by id renders whole rather than as a snippet. Agent filters use the fleet expression grammar — |, &, !, parens, names/ids/labels, and me — and A <> B reads messages BETWEEN two parties in both directions. A token one keystroke off a key is refused and told which key it meant, so sinse:2h says "did you mean since:" instead of being searched as the literal text sinse:2h; quote it to search it as text. cwd: is not a filter key and never was. since:/before: here take m as MINUTES, the same as the since/before parameters, and additionally accept 1w, 3mo, today, and yesterday, which the parameters do not.' },
