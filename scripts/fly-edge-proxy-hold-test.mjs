...
function startProxy({ listenPort, upstreamPort, holdSeconds, healthPort = 0 }) {
...
// --- 5. Health tells you WHICH half is broken.
//
// From outside, "the edge machine is gone" and "the app machine is gone" look
// the same: the tailnet name stops answering. During a cutover that is the only
// question worth asking, so health answers it on its own port — 200 while the
// app is reachable, 503 while it is not, and no reply at all if the edge itself
// is down.
{
  const listenPort = await freePort()
  const upstreamPort = await freePort()
  const healthPort = await freePort()
  const { child, lines } = startProxy({ listenPort, upstreamPort, holdSeconds: 20, healthPort })
  await waitForListening(lines)
  ...
  assert.equal(down.status, 503, 'no app machine means 503, not silence')
  assert.equal(down.body.edge, 'up', 'the edge saying "up" while reporting 503 is the whole point')
  assert.equal(down.body.up, false)

  const upstream = await startUpstream(upstreamPort, 'hello')
  const back = await health()
  assert.equal(back.status, 200)
  assert.equal(back.body.up, true)

  await stopUpstream(upstream)
  child.kill()
}

// --- 6. Every setting is required. A missing one must stop the process, not
//        become a guess about how long Skip waits.
for (const missing of ['TLDA_EDGE_LISTEN_PORT', 'TLDA_EDGE_UPSTREAM', 'TLDA_EDGE_HOLD_SECONDS', 'TLDA_EDGE_RETRY_MS', 'TLDA_EDGE_HEALTH_PORT']) {
  const env = {
    ...cleanEnv(),
    ...
    TLDA_EDGE_RETRY_MS: '100',
    TLDA_EDGE_HEALTH_PORT: String(await freePort()),
  }
  delete env[missing]
...
console.log('fly-edge-proxy: held through an absent upstream, survived a swap, carried a WebSocket, reported which half was down, expired its hold, and requires every setting')
